...
public class MatchRequestController implements MatchRequestApi {
	...
	@Override
	@PatchMapping("{matchRequestId}/retry")
	public ResponseEntity<ApiResponse<MatchRequestDto>> retry(
		@PathVariable Long matchRequestId,
		@AuthenticationPrincipal CustomUserDetails userDetails
	) {
		Long userId = userDetails.id();
		MatchRequestDto dto = matchRequestService.retry(userId, matchRequestId);
