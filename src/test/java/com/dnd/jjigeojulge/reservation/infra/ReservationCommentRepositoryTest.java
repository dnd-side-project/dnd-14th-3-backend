...
import com.dnd.jjigeojulge.global.config.AppConfig;
...
class ReservationCommentRepositoryTest {
        ...
        void saveAndFind() {
                ...
                ReservationComment foundComment = reservationCommentRepository.findById(savedComment.getId())
                                .orElseThrow();
        ...
        void findAllByReservationId() {
                ...
                reservationCommentRepository
                                .save(ReservationComment.create(reservation.getId(), 1L,
                                                CommentContent.from("첫 번째 댓글")));
                reservationCommentRepository
                                .save(ReservationComment.create(reservation.getId(), 2L,
                                                CommentContent.from("두 번째 댓글")));
        ...
        private Reservation createReservation() {
                ...
                return Reservation.create(ownerInfo, ReservationTitle.from("테스트 제목"), scheduledTime, placeInfo,
                                ShootingDurationOption.TEN_MINUTES, RequestMessage.from("요청사항 없음"));
        }
}
